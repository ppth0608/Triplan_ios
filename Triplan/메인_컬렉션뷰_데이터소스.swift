...
class 메인_컬렉션뷰_데이터소스: NSObject, UICollectionViewDataSource {
    ...
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        if let cell = collectionView.dequeueReusableCell(withReuseIdentifier: "여행추가", for: indexPath) as? 여행추가_셀 {
            let 여행추가인덱스임 = indexPath.item == 메인뷰모델.여행정보목록갯수
            if 여행추가인덱스임 {
                return cell
            }
            if let 여행정보 = 메인뷰모델.여행정보데이터(인덱스: indexPath.item) {
                if let cell = 여행정보.cell(from: collectionView, with: indexPath) {
